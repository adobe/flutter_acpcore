...
public class FlutterACPIdentityPlugin implements FlutterPlugin, MethodChannel.MethodCallHandler {
    ...
    public void onAttachedToEngine(@NonNull final FlutterPluginBinding binding) {
        channel = new MethodChannel(binding.getBinaryMessenger(), "flutter_acpidentity");
        ...
        Identity.getIdentifiers(new AdobeCallback<List<VisitorID>>() {
            ...
            public void call(List<VisitorID> visitorIDS) {
                ...
                for (VisitorID visitor: visitorIDS) {
                    ids.add(FlutterACPIdentityDataBridge.mapFromVisitorIdentifier(visitor));
                }
                AndroidUtil.runOnUIThread(new Runnable() {
                    @Override
                    public void run() {
    ...
    private void handleSyncIdentifier(final Object arguments) {
        ...
        VisitorID.AuthenticationState authenticationState = FlutterACPIdentityDataBridge.authenticationStateFromString((String) params.get("authState"));
